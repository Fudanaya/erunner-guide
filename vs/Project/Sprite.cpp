...
void Engine::Sprite::Update(float deltaTime)
{
	...
	//Added
	//Track direction
	static bool pingPongReversing = false;

	if (frameDuration >= maxFrameDuration) {
		frameDuration = 0;

		// Check if current animation should ping-pong
		if (currentAnim->pingPong) {
			if (pingPongReversing) {
				frameIndex--;  // Move backward

				if (frameIndex <= currentAnim->startFrameIndex) {
					frameIndex = currentAnim->startFrameIndex;  // Clamp to the start
					pingPongReversing = false;  // Switch direction
				}
			}
			else {
				frameIndex++;  // Move forward

				if (frameIndex >= currentAnim->endFrameIndex) {
					frameIndex = currentAnim->endFrameIndex;  // Clamp to the end
					pingPongReversing = true;  // Switch direction
				}
			}
		}
		else {
			// Regular animation behavior
			frameIndex = (frameIndex < currentAnim->startFrameIndex || frameIndex >= currentAnim->endFrameIndex) ?
				currentAnim->startFrameIndex : frameIndex + 1;
		}
	}
...
Engine::Sprite* Engine::Sprite::AddAnimation(string name, int startFrameIndex, int endFrameIndex, bool pingPong)
{
	AnimData* a = new AnimData();
	a->name = name;
	a->startFrameIndex = startFrameIndex;
...
//Added
Engine::Sprite* Engine::Sprite::ResetAnimFrame() {
	//Added
	frameIndex = currentAnim->startFrameIndex; // Reset to first frame
	frameDuration = 0; // Ensure animation starts from the beginning
	return this;
}
//End of added
